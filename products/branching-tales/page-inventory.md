...
| `/quests/[id]` | `pages/quests/[id]/` (planned) | `data.quests` + FKs + `data.items` (loot resolution) | Embedded `steps[]` and nested `steps[].rewards[]` (two-level), formulas (`total_xp`, `total_loot_xp`, `step_count`) | Draft |
...
| `/locations` | `pages/locations/` (planned) | `data.locations`, `data.regions` | FK to region, map image file | Draft |
| `/classes` | `pages/classes/` (planned) | `data.classes` | Small enum-like catalog | Draft |

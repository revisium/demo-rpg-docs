...
| Codex home hero (CMS) | `landing_hero.bg_image` | `pages/home/` | GraphQL | The CMS-vs-data subgraph attribution + the SSR-optimised image markup | Draft |
...
| `orderBy` with priority key (pinned-first pattern) | future `news.pinned desc, news.published_at desc` | `pages/news/` | GraphQL + REST | Blocked until a real `news` table exists; when added, the widget shows the priority-key `orderBy` JSON where `pinned` wins over natural time sort | Blocked |
| `where` with time-window filter | future `news.published_at <= now` | `pages/news/` | GraphQL + REST | Blocked until a real `news` table exists; when added, the widget shows the "no future-dated rows" predicate JSON and equivalent REST query string | Blocked |

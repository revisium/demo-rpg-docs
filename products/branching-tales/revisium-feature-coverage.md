...
| `orderBy` with priority key (pinned-first pattern) | `news.pinned desc, news.published_at desc` | `pages/news/` | GraphQL + REST | The pinned-news rendering on `/news` is driven by a multi-key `orderBy` where a `boolean` priority key wins over the natural time sort. Widget calls out that the pattern is schema-only, no query branching needed | Draft |
| `where` with time-window filter | `news.published_at <= now` | `pages/news/` | GraphQL + REST | The "no future-dated rows" rule is enforced via the GraphQL `where` clause; the widget shows the predicate JSON and the equivalent REST query string | Draft |

...
The demo needs to show three things at once:
...
A BFF that proxies Revisium internally was considered briefly. It would hide the very component the demo is meant to showcase, so it is rejected up front.
...
`demo-rpg-backend` wraps the Revisium client and exposes a single GraphQL schema. Rejected: hides Revisium and makes the demo less honest about how Revisium plugs in.

### Alternative B: Direct frontend → Revisium GraphQL

Frontend talks to `cloud.revisium.io` directly. Rejected: cannot show federation, cannot showcase the NestJS-subgraph patterns the DevRel audience expects.

### Alternative C: Federate `demo-rpg-data`, consume `demo-rpg-cms` directly

Two API paths: federation for game data, direct for CMS. Rejected: weaker demo narrative, two cache stories, and it forecloses future joins between editorial content and game entities.
...
- Schema composition errors if subgraphs drift. Mitigated by `supergraph-builder` retaining the last good supergraph until composition succeeds again.
